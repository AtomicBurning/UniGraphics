class Camera
{
    ...
    update(input, dt) {
        ...
        const up = normalize(cross(right, front));

        if (input.isDown("c") && this.canToggle)
        {
        ...
        {
            const offset = addVector(scaleVector(right, 1), addVector(scaleVector(up, 2), scaleVector(front, -5)));
            this.eye = addVector(this.target, offset);
        }
    }
